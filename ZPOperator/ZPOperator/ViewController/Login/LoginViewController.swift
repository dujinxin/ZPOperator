...
//        let ids = storyboard.instantiateViewController(withIdentifier: "IdentityAuthentication")
//        self.navigationController?.pushViewController(ids, animated: true)
//
//        return
        
        
//        userTextField.text = "15256984112"
//        passwordTextField.text = "12345678a"
...
//        return
        ...
        self.vm.login(userName: phone, password: password) { (data, msg, isSuccess) in
            self.hideMBProgressHUD()
            if isSuccess {
                self.dismiss(animated: true, completion: {
                    NotificationCenter.default.post(name: NSNotification.Name(rawValue: NotificationLoginStatus), object: true)
                })
//                if UserManager.manager.userAccound.isAuthed == 1 {
//                    NotificationCenter.default.post(name: NSNotification.Name(rawValue: NotificationLoginStatus), object: true)
...
extension LoginViewController: cwIntegrationLivessDelegate{
    func cwIntergrationLivess(_ isAlive: Bool, errorCode code: Int, bestFaceImage bestFaceData: Data?, encryptJson jsonStr: String?) {
